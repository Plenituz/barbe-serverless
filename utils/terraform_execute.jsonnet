...
local pipeline = [
    ...
    function(container) barbe.databags([
        barbe.iterateBlocks(container, "terraform_empty_execute", function(bag)
            ...
            assert mode == "apply" || mode == "destroy": "terraform_execute mode must be either 'apply' or 'destroy'";
            {
                ...
                Value: {
                    ...
                    dockerfile: |||
                       ...
                       RUN printf %(template)s > ./template.tf.json
                       RUN terraform init -input=false
                       RUN terraform %(command)s -auto-approve -input=false %(extra_args)s
                       RUN printf %(output)s > tf_output.json
                    ...
                    ||| % {
                        ...
                        id: bag.Name,
                        command: mode,
                        template: std.escapeStringJson(barbe.asStr(block.template_json)),
                        output: std.escapeStringJson(std.manifestJsonMinified({
